...
def make_predictions(input_data_dict, models, X_train):
    ...
    # ✅ Add Suggested Mindfulness Activity to avoid KeyError
    mindfulness_options = ['Meditation', 'Deep Breathing', 'Yoga', 'Nature Walk', 'Reading']
    all_predictions['Suggested Mindfulness Activity'] = random.choice(mindfulness_options)

    # ✅ Optional: Add Suggested Supplements
    supplements_options = [['Vitamin D', 'Omega-3'], ['Magnesium', 'Zinc'], ['Probiotics'], [], ['Multivitamin']]
    selected_supplements = random.choice(supplements_options)
    all_predictions['Suggested Supplements'] = ', '.join(selected_supplements) if selected_supplements else None
...
""", unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.markdown("Built with ❤ using Python, Scikit-learn, and Streamlit")
